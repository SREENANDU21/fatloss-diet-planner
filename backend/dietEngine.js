...
const parseIngredients = (mealName, scalingFactor) => {
    ...
    if (name.includes("idli") || name.includes("dosa") || name.includes("uttapam")) add("Idli/Dosa Batter", 150, "g");
    if (name.includes("rice") || name.includes("chawal") || name.includes("bhaat") || name.includes("jadoh") || name.includes("bath") || name.includes("khichdi")) add("Rice (Raw)", 60, "g");
    if (name.includes("roti") || name.includes("chapati") || name.includes("thepla") || name.includes("paratha") || name.includes("litti")) add("Whole Wheat Flour", 70, "g");
    if (name.includes("bhakri") || name.includes("mudde")) add("Millet Flour (Jowar/Bajra/Ragi)", 70, "g");
    if (name.includes("chicken")) add("Chicken Breast/Lean Meat", 150, "g");
    if (name.includes("fish") || name.includes("macher")) add("Fish", 150, "g");
    if (name.includes("pork")) add("Pork (Lean cuts)", 150, "g");
    if (name.includes("dal") || name.includes("sambar") || name.includes("kadhi") || name.includes("pitla") || name.includes("zunka")) add("Lentils (Toor/Moong/Masoor)", 50, "g");
    if (name.includes("paneer")) add("Paneer (Cottage Cheese)", 100, "g");
    if (name.includes("veg") || name.includes("sabzi") || name.includes("poriyal") || name.includes("stew") || name.includes("tarkari")) add("Mixed Vegetables", 150, "g");
    if (name.includes("poha") || name.includes("chida")) add("Poha (Flattened Rice)", 60, "g");
    if (name.includes("oats")) add("Rolled Oats", 50, "g");
    if (name.includes("fruit")) add("Mixed Fruits", 150, "g");
    if (name.includes("chana") || name.includes("sundal") || name.includes("ghugni") || name.includes("misal")) add("Chana/Chickpeas/Sprouts", 50, "g");
